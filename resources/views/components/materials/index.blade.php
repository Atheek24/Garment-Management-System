...
@section('content')
    <section class="container mx-auto px-4">
        <h2 class="text-lg font-medium text-gray-800 dark:text-white">Materials</h2>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-300">Manage your materials inventory effectively.</p>
                ...
                <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
                    <div class="overflow-hidden border border-gray-200 md:rounded-lg dark:border-gray-700">
                        ...
                        <div
                            ...
                            <h3 class="text-lg font-semibold text-gray-800 dark:text-white">Material List</h3>
                            @if (in_array(Auth::user()->role, ['Super Admin', 'Manager', 'Inventory Manager']))
                                    ...
                                    @foreach ($all_materials as $material)
                                        <tr>
                                            ...
                                            <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-200">
                                                {{ $material->name }}</td>
                                            <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-200">
                                                {{ $material->unitCost }}</td>
                                            <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-200">
                                                {{ $material->quantityInStock }}</td>
                                            <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-200">
                                                {{ $material->unit }}</td>
                                            @if (in_array(Auth::user()->role, ['Super Admin', 'Manager', 'Inventory Manager']))
                                                <td class="px-4 py-3 text-sm">
                                                    <button onclick="openModal('editMaterialModal-{{ $material->id }}')"
                                                        class="text-blue-500 hover:underline">Edit</button>
                                                </td>
                                                <td class="px-4 py-3 text-sm">
                                                    <a href="/material/delete/{{ $material->id }}"
                                        ...
                                        <x-materials.edit id="editMaterialModal-{{ $material->id }}" :material="$material" />
                                    @endforeach
                                @else
                                    <tr>
                                        <td colspan="7"
    ...
    <x-materials.add id="addMaterialModal" />
